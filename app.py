...
def load_model():
    ...
    if not os.path.exists(path):
        st.error(f"Файл модели '{path}' не найден. Положите его в папку с приложением.")
        st.stop()
    
    try:
        # Добавляем compile=False для обхода ошибок десериализации в Keras 3
        # Это позволяет загрузить веса и архитектуру без попытки восстановить оптимизатор
        model = tf.keras.models.load_model(path, compile=False)
        return model
    except Exception as e:
        st.error("⚠️ Ошибка при загрузке модели!")
        st.markdown(f"**Детали ошибки:** `{str(e)}`")
        st.info("Это часто случается из-за разницы версий TensorFlow/Keras. Попробуйте пересохранить модель в формате .keras в актуальной версии Keras.")
        st.stop()
